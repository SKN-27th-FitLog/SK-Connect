"""PA-L0-SHP: get_reviews._filter_rows_by_shop_match (Level 0, DB 미접촉)."""
...
def _shop_1to1() -> pd.DataFrame:
    """shop map_id 100 → 1:1 매칭 fixture."""
    return pd.DataFrame(
        {
...
def test_pa_l0_shp_001_empty_crawling() -> None:
    """PA-L0-SHP-001 [경계]: crawling 빈 DataFrame → 빈 결과."""
...
def test_pa_l0_shp_002_one_to_one_match() -> None:
    """PA-L0-SHP-002 [정상]: 1:1 shop 매칭 시 shop_id·shop_cd 부여."""
    out = _filter_rows_by_shop_match(
        _crawl_row(1, 100),
        _shop_1to1(),
        crawling_id_col=CrawlingColumn.CRAWLING_ID.value,
...
def test_pa_l0_shp_003_null_map_id_dropped() -> None:
    """PA-L0-SHP-003 [실패]: map_id 결측 행 제외."""
    out = _filter_rows_by_shop_match(
        _crawl_row(2, None),
        _shop_1to1(),
        crawling_id_col=CrawlingColumn.CRAWLING_ID.value,
...
def test_pa_l0_shp_004_unknown_map_id_dropped() -> None:
    """PA-L0-SHP-004 [실패]: shop 0건 map_id 제외."""
    out = _filter_rows_by_shop_match(
        _crawl_row(3, 999),
        _shop_1to1(),
        crawling_id_col=CrawlingColumn.CRAWLING_ID.value,
...
def test_pa_l0_shp_005_ambiguous_shop_dropped() -> None:
    """PA-L0-SHP-005 [실패]: shop 2건 이상 map_id 제외."""
    out = _filter_rows_by_shop_match(
        _crawl_row(4, 200),
        _shop_ambiguous(),
        crawling_id_col=CrawlingColumn.CRAWLING_ID.value,
...
def test_pa_l0_shp_006_string_map_id_normalized() -> None:
    """PA-L0-SHP-006 [불변]: map_id 문자열을 numeric으로 정규화 후 매칭."""
    out = _filter_rows_by_shop_match(
        _crawl_row(5, "100"),
        _shop_1to1(),
        crawling_id_col=CrawlingColumn.CRAWLING_ID.value,
...
def test_pa_l0_shp_007_warns_on_null_map_id(caplog: pytest.LogCaptureFixture) -> None:
    """PA-L0-SHP-007 [정상]: map_id 결측 시 shop 미매칭 warning 로그."""
