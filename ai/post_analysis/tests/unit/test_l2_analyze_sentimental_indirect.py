...
@patch("analyze_sentimental.BertTokenizer")
@patch("analyze_sentimental.get_analysis_data")
def test_pa_l2_snt_004_excludes_empty_content(
    mock_get: MagicMock,
    mock_bert_cls: MagicMock,
    mock_merge: MagicMock,
) -> None:
    """PA-L2-SNT-004 [불변]: 빈 content pending 행은 제외, 유효 본문만 predict."""
    mock_get.return_value = pd.DataFrame(
        {
    ...
    df = mock_merge.call_args[0][0]
    assert len(df) == 1
    mock_inst.predict_sentiment.assert_called_once_with("맛있다")
