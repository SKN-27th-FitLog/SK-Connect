...
2. 기대결과 : 
> 2.1) (x, y) 기준으로 요청한 값 범위 내 게시글 목록 반환
> 2.2) 게시글 목록은 ID / 카테고리 / (x, y) 좌표 까지만 받음
> 2.3) 게시글은 원점에서 직선거리 ( math.sqrt( x\\**2 + Y\\**2) 처럼 처리되어야 함) 오름차 순 정렬 후 일정 개수 만큼 반환 
> 2.4) 반환 개수는 사이즈가 고정되는게 좋을 것 같으면 약 100 ~ 200개 정도로 그냥 확정해도 됨 
> 2.5) 게시글 구하는 조건은 요청에 포함될 원점에서 벗어난 반경 범위로 계산
> 2.6) 지도 정보가 없는 데이터는 제외

3. api 요청 시 추가 인자
> 1. 지도 중심 위치좌표 : [x:float, y:float] 
> 2. 반환 요청할 리스트 개수 : int # 개수 지정 안하는게 나을 것 같으면 고정 호출이어도 됨 
> 3. 지도 중심 원점에서 어느정도 떨어진 거리 요청할지 : float


4. 받을 데이터 포멧 (json) -> 반경 범위 내의 마커 중 직선거리 순으로 오름차순 정렬 후 일정개수 반환 
> 1. 게시글 ID : int 
> 2. 게시글 위치좌표 : [x:float , y:float ]
> 3. 게시글 위치 이름 : str
> 4. 카테고리 이름 : enum 
...
## 게시글 마크 정보 요청
1. 용도 : 
> 1.1) 지도 화면 특정 마크 클릭 시 게시글 ID에 해당하는 요약 정보를 호출하는 용도 


2. 기대결과 : 
> 2.1) 지도에 표시할 용도로 특정ID의 게시글에 요약정보 조회 요청 -> 전달받은 ID의 게시글 요약정보 1개 전달 
> 2.2) 위치 정보는 받지 않아도 됨 (이미 알고 있음 )
> 2.3) 카테고리도 받지 않아도 됨 (이미 알고 있음 )

3. api 요청 시 추가 인자
> 1. 게시글 ID : int 


4. 받을 데이터 포멧 (json) -> 지정한 게시글의 제목 일부 / 내용 일부만 받으면 됨 
> 1. 제목 (30자) : str # 표시창이 작아서 출력 내용 일부만 받으면 됨 
> 2. 내용(120자) : str # 표시창이 작아서 출력 내용 일부만 받으면 됨 
