...
사용 예:
    python gatter_content_geeknews.py -i thread_geeknews.csv
    python gatter_content_geeknews.py -i thread_geeknews.csv -o out.csv --limit 3 --timeout 45
"""
...
def _fetch_text(url: str, timeout: float, *, user_agent: str = _DEFAULT_USER_AGENT) -> str:
    '''GET으로 URL HTML을 받아 UTF-8(오류 시 replace)로 디코드한 문자열을 반환한다.'''
...
class _HTMLToText(HTMLParser):
    """토픽 본문 HTML 조각을 공백 위주로 이어 붙인 플레인 텍스트로 변환."""
...
def extract_topic_body(html: str) -> str | None:
    """토픽 페이지 HTML에서 본문만 플레인 텍스트로 반환. 실패 시 None."""
...
def main() -> int:
    '''CLI: 입력 CSV를 읽어 article_url별 본문을 채운 뒤 _with_content CSV로 저장한다. 오류 시 비제로 종료.'''
