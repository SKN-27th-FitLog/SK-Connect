...
class Stage1RawCollection(BaseStage):
    ...
    async def execute_shard(
        self, shard_targets: List[Dict[str, Any]], batch_id: str, category_cd: str, shard_id: str, run_attempt: int = 1
    ) -> List[Dict[str, Any]]:
        results = []
        now = datetime.now()
        ...
        for target in shard_targets:
            ...
            if search_query and hasattr(self.collector, 'discover_stores'):
                ...
                for url in urls:
                    res = await self._collect_single_url(
                        url, target.get('address_cd'), batch_id, category_cd, shard_id, now, run_attempt
                    )
                    results.append(res)
            ...
            # 2. 직접 URL 수집
                ...
                res = await self._collect_single_url(
                    target['url'], target.get('target_id', 'unknown'), batch_id, category_cd, shard_id, now, run_attempt
                )
                results.append(res)
    ...
    async def execute(self, targets: List[Dict[str, Any]], batch_id: str, category_cd: str, run_attempt: int = 1) -> List[Dict[str, Any]]:
        ...
        async def _run_shard_with_limit(shard_id: str, shard_data: List[Dict[str, Any]]):
            async with sem:
                return await self.execute_shard(shard_data, batch_id, category_cd, shard_id, run_attempt)
